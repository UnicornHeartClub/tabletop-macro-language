...
const Executor = (options) => {
  ...
  // A dictionary of user-defined functions
  executor.functions = {}
      ...
      if (isDeep) {
        ...
        attrs.forEach((attr) => {
          ...
          if (!isNumber) {
            ...
            ref = ref.Array[attr]
          } else if (isNumber && ref[attr]) {
            // if we're here, we've referenced a deeply nested token
            ref = setRef ? ref[attr] : ref[attr].attributes
          }
...
export function executeStep (step = {}, output) {
  if (typeof step.op === 'object' && step.op.Function) {
    // execute a user defined function
    return executeStepFunction(step, output)
  }
...
export async function executeStepFunction (step = {}, output) {
  return new Promise(async (resolve, reject) => {
    try {
      const name = step.op.Function

      if (!executor.functions[name]) {
        return reject(new Error(`No defined function "${name}"`))
      }

      const args = step.args.map(arg => getArgValue(arg.Function))
      const result = await executor.functions[name](...args)
      const length = Object.keys(executor.variables).length
      executor.variables[`${length + 1}`] = result

      resolve(result)
    } catch (error) {
      reject(new Error('Defined function encountered an error or was cancelled'))
    }
  })
}
...
export async function executeStepLambda (step = {}, output) {
  return Promise.each(step.args, (arg) => new Promise(async (resolve, reject) => {
    try {
      if (typeof arg.Conditional !== 'undefined') {
        ...
        const result = await executeStep(eq ? success : failure, output)
        return resolve(result)
      } else if (arg.Assign || arg.Concat || arg.Deduct) {
        const assign = arg.Assign || arg.Concat || arg.Deduct
        const left = assign.left

        // if we have a step, run the step and get the immediate output
        const right = await Promise.all(assign.right.map(val => new Promise(async (res, rej) => {
          try {
            if (val.Step === undefined) {
              const argVal = getArgValue(val)
              res(argVal)
            } else {
                const result = await executeStep(val.Step, output)
          ...
          if (isVariable) {
            ...
            if (arg.Concat) {
              ...
              if (currentVal.Array) {
                ...
                // we have an object to append to
                const length = Object.keys(newVal).length
                newVal[length] = buildToken(result)
              }
        ...
        if (executor.tokens[name]) {
          ...
          if (token.macros[macroName]) {
            ...
            let exit = false
            const results = await Promise.each(inlineProgram.steps, (step) => {
              // If we encounter an exit, stop processing
              if (step.op === 'Exit') {
                exit = true
              }
        ...
        return reject(new Error(`Token macro @${name}->${macroName} not found`))
      }
    ...
    try {
      ...
      step.args.forEach((arg) => {
        if (typeof arg.Roll !== 'undefined') {
          ...
          if (rollArg === 'Advantage') {
            command += 'adv'
          } else if (rollArg === 'Disadvantage') {
            command += 'dis'
    ...
    if (executor.prompt) {
      try {
        ...
        const displayOptions = options.map((option, i) => {
          ...
          if (defaultValue !== undefined && String(defaultValue) === key) {
            defaultResult = i
          }
