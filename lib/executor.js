...
export function executeStepPrompt (step = {}, output) {
  return new Promise(async (resolve, reject) => {
    if (executor.prompt) {
      ...
      let options = {}
      step.args.forEach((arg) => {
        if (typeof arg.Prompt.message === 'object') {
          message = getArgValue({ TextInterpolated: arg.Prompt.message })
        }
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      step.args.forEach((arg) => {
        ...
        } else if (typeof arg.Template.Attributes === 'object') {
          Object.keys(arg.Template.Attributes).forEach((attr) => {
            template.attributes[attr] = getArgValue(arg.Template.Attributes[attr])
          })
        }
      })
