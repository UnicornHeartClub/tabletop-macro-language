...
const Executor = (options) => {
  ...
  executor.macros = options.macros || {}
  executor.tokens = options.tokens || {}
  executor.prompt = options.prompt || {}
  executor.target = options.target || {}
  ...
  return Promise.each(step.args, (arg) => {
    if (typeof arg.Conditional !== 'undefined') {
      ...
      if (isVariable || isToken) {
        ...
        if (isVariable) {
          output.results[left.Variable] = result
        } else if (isToken) {
...
export function executeStepRoll (step = {}, output) {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      if (step.result === 'Save') {
        const length = Object.keys(output.variables).length
        output.variables[`${length + 1}`] = roll.value
      }
      resolve(roll)
    } catch (error) {
      reject(error)
