...
export const TTML = {
  /**
   * Add a callback for a user defined function
   * @param {String} name; Function name available in the macro language
   * @param {Function<Promise>} action; The action to be run when function is called
   * @return void
   */
  addFunction: (name, action = (input) => {}) => {
    executor.functions[name] = action
    return executor.functions[name]
  },
