...
test('.buildToken sets correct data structure', (t) => {
  ...
  const buildToken = TTML.buildToken(token)
  t.deepEqual(buildToken, expectedToken)
})
...
test('.unbuildToken returns deconstructed data structure', (t) => {
  ...
  const buildToken = TTML.unbuildToken(token)
  t.deepEqual(buildToken, expectedToken)
})
...
test('it pushes elements to a token attribute array', async (t) => {
  ...
  TTML.setToken('me', token, {})
  await execute([ assignStep ], false)
  t.is(executor.tokens.me.attributes.attacks.Array.length, 2)
  t.deepEqual(executor.tokens.me.attributes.attacks.Array[1], {
    name: { Text: 'test' },
  })
})
