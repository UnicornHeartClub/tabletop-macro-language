...
test('it executes a custom function step', async (t) => {
  TTML.setToken('me', token, {})
  executor.functions.get = sinon.stub().resolves({ name: 'Sword of Awesome' })
  const step = {
    args: [
      {
        Concat: {
...
test.only('it executes a Roll advantage', async (t) => {
  const stepRollAdv = {
    args: [
      { Roll: 'Advantage' },
    ],
  ...
  const output = await execute([ stepRollAdv ], false)
  t.true(executor.api.calledWith('adv'))
})

test.only('it executes a Roll disadvantage', async (t) => {
  const stepRollAdv = {
    args: [
      { Roll: 'Disadvantage' },
    ],
  ...
  const output = await execute([ stepRollAdv ], false)
  t.true(executor.api.calledWith('dis'))
})
